...
@Injectable()
export class TenantSetup_Service {
    ...
    save(tenant_main: TenantSetup_Model): Observable<any> {
    ...
    update(tenant_main: TenantSetup_Model): Observable<any> {
    ...
    get(id: string, params?: URLSearchParams): Observable<TenantSetup_Model> {        
        var queryHeaders = new Headers();
        queryHeaders.append('Content-Type', 'application/json');
        //queryHeaders.append('X-Dreamfactory-Session-Token', localStorage.getItem('session_token'));
        queryHeaders.append('X-Dreamfactory-API-Key', constants.DREAMFACTORY_API_KEY);
//alert(id);
        return this.httpService.http
            ...
            .map((response) => {
                var result: any = response.json();
                let tenanttype: TenantSetup_Model = TenantSetup_Model.fromJson(result);
                return tenanttype;
            }).catch(this.handleError);
    };
}
